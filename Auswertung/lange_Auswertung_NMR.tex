...
\bibliography{lit_afm} %Neues Quellenverzeichnis einfügen!
...
Um ein positionsaufgelöstes NMR-Signal aufzunehmen, müssen Magnetfelder verwendet werden, die eine Ortsabhängigkeit besitzen. Hierzu werdem dem statischen Feld $\vec{B_0}$, welches die z-Achse definiert, Gradientenfelder überlagert. Diese Magnetfelder $\vec{B^x}, \vec{B^y}, \vec{B^z}$ zeigen in z-Richtung und die Feldstärke steigt linear als Funktion der jeweiligen Koordinate an.

\paragraph{1-Dimensionale Bildgebung}
...
\paragraph{2-Dimensionale Bildgebung}
